...
export class WeaponDynamic implements  OnChanges, DoCheck {

	@Input() weapDynamic: NinjaWeapon[]; 
	@Output() weapDelete: EventEmitter<number> = new EventEmitter<number>();
	
	constructor(private differs: KeyValueDiffers){
		console.log('Created new WeaponDynamic', new Date());
		this.differ = differs.find({}).create(null);
	}
	
	removeSingleTrack(){
		this.weapDelete.emit({id: this.weapDynamic.id});
	}
